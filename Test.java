...
public class Test {
	...
	private static void testTowerSolve() {
		...
		for (int i = 0; i <= n; i++) {
			...
			if (stealer.printouts.size() != (int) Math.pow(2, i)) {
				System.setOut(out);
				System.out.println("Failed: t.solve failed to solve the puzzle 3, " + i + " in " + (int) Math.pow(2, i)
						+ " turns. " + stealer.printouts.size() + " turns were used.");
				return;
			}
		}
				...
				for (int k = 0; k < n - (i + j); k++) {
					...
					for (int r = n - 1; r >= 0; r--) {
						for (int c = 0; c < 3; c++) {
							...
							if (r < getRodNum(rod)) {
								int spaces = ((disks.length * 2 + 1) - disks[r].toString().length()) / 2;
								int dsize = getDiskSize(disks[r]);
								re = re + "[ ]{" + spaces + "}[<]{1}[=]{" + (dsize - 1) + "}[" + dsize + "]{1}[=]{"
										+ (dsize - 1) + "}[>]{1}[ ]{" + spaces + "}";
							} else {
					...
					if (!tres.matches(re)) {
						if (tres.charAt(tres.length() - 1) != '\n') {
							System.out.println("Failed: TowerOfHanoi.toString() must end with a \\n character");
							return;
						}
			...
			for (int j = 0; j < n; j++) {
				...
				if (fail) {
					...
					if (!stealer.getLast().equals("WARNING: Illegal move.\n")) {
						System.setOut(out);
						System.out.println(
								"Failed: Did not print out \"WARNING: Illegal move.\" where compareTo was " + c);
						return;
					}
					...
					if (!stealer.getLast().equals("")) {
						...
						System.out.println("Failed: Printed out \"" + stealer.getLast()
								+ "\" where nothing should have been printed, since compareTo was " + c);
						return;
					}
		...
		for (int i = 1; i <= 9; i++) {
			for (int j = 0; j <= 9; j++) {
				...
				try {
					...
					System.out.println("Failed: The TowerOfHanoi constructor threw an exception when given width = " + i
							+ " and height = " + j + "; this is not supposed happen.");
				...
				if (t != null) {
					Rod[] rods = getTowerRods(t);
					if (rods == null) {
						System.out.println("Failed: TowerOfHanoi.rods was found to be null or not declared.");
						return;
					}
					if (rods.length != i) {
						System.out.println("Failed: The length of the rods array is " + rods.length
								+ ", but the width passed in was + " + i + "; The constructor was given width = " + i
								+ " and height = " + j);
						return;
					}
					for (int k = 0; k < i; k++) {
						...
						if (ds.length != j) {
							System.out.println("Failed: The height of the rod at position " + k + " is " + ds.length
									+ ", but the height passed in was + " + j + "; The constructor was given width = "
									+ i + " and height = " + j);
							return;
						}
						if (k == 0 && !r.isFull()) {
							System.out.println(
									"Failed: The first rod is not full, and its contents are " + Arrays.toString(ds)
											+ "; The constructor was given width = " + i + " and height = " + j);
							return;
						}
						if (k > 0 && !r.isEmpty()) {
							System.out.println("Failed: The rod after the first rod, at position " + k
									+ ", is not empty, and its contents are " + Arrays.toString(ds)
									+ "; The constructor was given width = " + i + " and height = " + j);
							return;
						}
					}
		...
		for (int i = 1; i <= 9; i++) {
			for (int j = 0; j < i; j++) {
				...
				if (!rres.matches(re)) {
					...
					System.out.println("Failed: Rod.toString() on Rod with maxHeight = " + i + " and numberOfDisks = "
							+ j + " gave the wrong output:");
	...
	private static void testRodCompare() {
		...
		try {
			...
			if (n != 1) {
				System.out.println("Failed: compareTo(empty) when not empty returned " + n + ", not 1");
				return;
			}
			r = new Rod(3, 2);
			r.pop();
			...
			System.out.println("Failed: NullPointerException was thrown. Are you checking for null?");
			return;
		}
		System.out.println("Passed.");
	}
	...
	private static void testRodPeek() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (!caught) {
				System.out.println("Failed: did not throw NoSuchElementException when peeking on an empty rod.");
				return;
			}
		}
		for (int i = 1; i < 10; i++) {
			...
			for (int j = 0; j < i; j++) {
				...
				if (rpeek != speek) {
					System.out.println("Failed: Peek returned the wrong element, expected " + speek + ", got " + rpeek);
					return;
				}
				// check equality
				if (!testEquality(r, s)) {
				...
				if (s.contains(null)) {
					System.out.println("Failed: Rod contains null elements in positions under numberOfDisks: ");
					System.out.println(stackToRodList(s, i));
					return;
				}
			}
	...
	private static void testRodPop() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (!caught) {
				System.out.println("Failed: did not throw NoSuchElementException when popping from an empty rod.");
				return;
			}
		}
		for (int i = 1; i < 10; i++) {
			...
			for (int j = 0; j < i; j++) {
				try {
					...
					if (rpop != spop) {
						System.out
								.println("Failed: Pop returned the wrong element, expected " + spop + ", got " + rpop);
						return;
					}
				} catch (Exception e) {
	...
	private static void testRodPush() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (!caught) {
				System.out.println("Failed: did not throw IllegalStateException when pushing onto a full rod.");
				return;
			}
		}
		for (int i = 1; i < 10; i++) {
	...
	private static void testRodEmpty() {
		...
		try {
			for (int i = 1; i < s; i++) {
				...
				if (r.isEmpty()) {
					System.out
							.println("Failed: Rod should not be empty, but isEmpty returned true on expected contents "
	...
	private static void testRodFull() {
		...
		try {
			for (int i = 1; i < s; i++) {
				if (r.isFull()) {
					System.out.println("Failed: Rod should not be full, but isFull returned on expected contents "
							+ stackToRodList(st, s).toString());
					return;
				}
				Disk d = new Disk(i);
			...
			for (int i = 1; i < s; i++) {
				r.pop();
				if (r.isFull()) {
					System.out.println("Failed: Rod should not be full, but isFull returned true on expected contents "
							+ stackToRodList(st, s).toString());
					return;
				}
			}
	...
	private static boolean testEquality(Rod r, Stack<Disk> s) {
		...
		if (s.size() != getRodNum(r)) {
			System.out.println("Failed: Expected size " + s.size() + ", but rod size was " + getRodNum(r));
			return false;
		}
		// comparison
		...
		if (!result) {
			System.out
					.println("Failed: Expected contents " + st.toString() + ", but rod contents were " + rt.toString());
		}
		return true;
	}
		...
		for (int i = 1; i < 10; i++) {
			for (int j = 0; j < i; j++) {
				...
				try {
					...
					System.out.println("Failed: Rod constructor threw an exception on input maxHeight = " + i
							+ " and numberOfDisks = " + j);
					return;
				}
				if (r != null) {
					try {
						...
						Disk[] disks = getRodDisks(r);
						if (disks.length != i) {
							System.out.println("Failed: Rod given maxHeight = " + i + " and numberOfDisks = " + j
									+ " has maximum height " + disks.length + "");
							return;
						}
						if (num != j) {
							System.out.println("Failed: Rod given maxHeight = " + i + " and numberOfDisks = " + j
						...
						for (int k = 0; k < i; k++) {
							...
							if (k < j) {
								...
								if (size != j - k) {
									System.out.println("Failed: Rod given maxHeight = " + i + " and numberOfDisks = "
											+ j + " has an element of size " + size + " at position " + k
											+ " where it should be of size " + (j - k));
									return;
								}
							}
						}
	...
	private static void testDiskString() {
		...
		for (int i = 1; i < 9; i++) {
			...
			if (!s.matches(re)) {
				System.out.println("Failed: Disk.toString() on size " + i + " gave the wrong output: " + s);
				return;
			}
		}
		System.out.println("Passed.");
	...
	private static void testDiskConstr() {
		...
		for (int i = -1; i <= 11; i++) {
			...
			if (shouldFail != failed) {
				if (shouldFail) {
					System.out.println("Failed: Did not throw IllegalArgumentException when size = " + i + "");
					return;
				} else {
					System.out.println("Failed: Threw IllegalArgumentException when size = " + i + "");
					return;
				}
			}
			...
			if (d != null) {
				...
				if (size != i) {
					System.out.println(
							"Failed: The field 'size' is equal to " + size + ", not the expected value of " + i);
					return;
				}
			}
		}
